...
def integration(folder):
    ...
    for file in os.listdir(folder):
        ...
        if file_path.endswith('.csv'):  # Only process CSV files
            processed_data = pp(file_path)  # Pass the cutoff_value
...
def main():
    ...
    file = './data/GSE171524_RAW'
    output_folder = 'output'
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)   
    # Check if data folder exists before proceeding
    if not os.path.exists(file):
        print(f"Error: File '{file}' does not exist. Please check the extraction.")
        return
    # Step 2: Run parameter adjustments
    parameter_adjust(file)
